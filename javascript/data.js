$("document").ready(function () {
    ...
    //add the items into the UL
    var artists_ul = document.getElementById("artists-ul");
    ...
    function buildListItems(liObject) {
        // variable for building the list
        var output = "";
        for (var i in liObject) {
            output += "<li>";
            output += '<a href="' + liObject[i].link + '">';
            output += '<img src="' + liObject[i].img + '"/>';
            output += "<h3>" + liObject[i].fullName + " </h3>";
            output += "<h4>" + liObject[i].years + "</h4>";
            //output += "</a>";
            //output += '<a href="#">info</a>';
            output += "</li>";
        }

        artists_ul.innerHTML = output;
    }
